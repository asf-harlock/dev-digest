...
export const PromptAssembly = z.object({
  ...
  /** Declared intent & scope (Intent Layer); null when the PR has no
   *  classified intent. */
...
export const RunStats = z.object({
  ...
  /** USD spent on the run. `nullish` on purpose: run_traces documents written
      before this field existed have no such key and must keep parsing. */
...
export const RunSummary = z.object({
  ...
  /** USD spent on the run; null when unknown — the UI shows "—", never "$0.00". */
